...
class Slider_Model extends Model {
    ...
    public function listItems() {
        try{
            ...
            $query = "SELECT P.`id`, P.`ordering`, PT.`title`, A.`name` as `creater_name`, P.`create_date`, P.`status`  
                        FROM `cms_slider` P
                        LEFT JOIN `cms_slider_text` PT ON PT.`p_id` = P.`id` and PT.lang=:lang  
    ...
    public function updateStatus($id, $post=[]) {
        ...
        try{
            ...
            if($status) {
                ...
                $result = $this->db->update('`cms_slider`', $updateData, "`id`={$id}");
                Func::dbError($result, __LINE__);
    ...
    public function orderingItem($items) {
        try{
            foreach ($items as $key => $value) {
                ...
                $result = $this->db->update('`cms_slider`', $updateData, "`id`={$key}");
                Func::dbError($result, __LINE__);
            }
            return 1;
        } catch (Exception $e) {
    ...
    public function createItem() {
        try{
            ...
            foreach (MFAdmin::$_langs as $key=>$value){
                ...
                $this->form->post('data_link_'.$key)->filter('filter_string');
...
//            exit;
            ...
            if(!empty($data)){
                ...
                $last_id = $this->db->insert('cms_slider', $insertData);
                ...
                if (ctype_digit($last_id) === true) {
                    ...
                    if(!empty($_FILES['data_photo']['name'])){
                        $uploadPhoto = Func::uploadPhoto($_FILES['data_photo'], $this->_menu, $last_id, Func::rund_number(), slider_width, slider_height, 1);
                        if(isset($uploadPhoto['error']) && !empty($uploadPhoto['error'])){
                            ...
                            $updateResult = $this->db->update('`cms_slider`', $updateData, "`id`={$last_id}");
                    ...
                    foreach (MFAdmin::$_langs as $key=>$value){
                        ...
                        $insertResult = $this->db->insert('cms_slider_text', $insertData);
    ...
    public function singleItem($id) {
        try{
            ...
            $query = "SELECT S.`id`, S.`ordering`, S.`photo`, S.`status`, T.`title`, T.`subtitle`, T.`link`, T.`lang` 
                            FROM `cms_slider` S 
                            LEFT JOIN `cms_slider_text` T ON T.p_id = S.id   
                            WHERE S.`id`=:id";
            $result = $this->db->select($query, $selectData);
            Func::dbError($result, __LINE__);
...
//            exit;
            ...
            if (!empty($data)){
                ...
                if($result === 1){
                    // upload photo
                    if(!empty($_FILES['data_photo']['name'])){
                        $uploadPhoto = Func::uploadPhoto($_FILES['data_photo'], $this->_menu, $id, Func::rund_number(), slider_width, slider_height, 1);
                        if(isset($uploadPhoto['error']) && !empty($uploadPhoto['error'])){
                            $error['headerError'] = $uploadPhoto['error'];
                            throw new Exception(json_encode($error));
                        } else {
                            ...
                            $updateResult = $this->db->update('`cms_slider`', $updateData, "`id`={$id}");
                            Func::dbError($updateResult, __LINE__);
                        }
                    }

                    // delete from database
                    $deleteResult = $this->db->delete('cms_slider_text',"`p_id` ={$id}");
