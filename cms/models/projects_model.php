    ...
    public function listItems() {
        try{
            ...
            $query = "SELECT P.`id`, PT.`title`, A.`name` as `creater_name`, P.`create_date`, P.`status`  
                        FROM `cms_projects` P
                        LEFT JOIN `cms_projects_text` PT ON PT.`p_id` = P.`id` and PT.lang=:lang  
    ...
    public function updateStatus($id, $post=[]) {
        ...
        try{
            ...
            if($status) {
                ...
                $result = $this->db->update('`cms_projects`', $updateData, "`id`={$id}");
                Func::dbError($result, __LINE__);
...
//            exit;
            ...
            if(!empty($data)){
                ...
                $last_id = $this->db->insert('cms_projects', $insertData);
                ...
                if (ctype_digit($last_id) === true) {
                    ...
                    $updateResult = $this->db->update('`cms_projects_photo`', $updateData, "`secret_id`='{$data['sid']}'");
                    ...
                    foreach (MFAdmin::$_langs as $key=>$value){
                        ...
                        $insertResult = $this->db->insert('cms_projects_text', $insertData);
    ...
    public function singleItem($id) {
        try{
            ...
            $query = "SELECT S.`id`, S.`secret_id`, S.`post_date`, S.`status`, T.`title`, T.`text`, T.`lang` 
                            FROM `cms_projects` S 
                            LEFT JOIN `cms_projects_text` T ON T.p_id = S.id   
                            WHERE S.`id`=:id";
            $result = $this->db->select($query, $selectData);
            Func::dbError($result, __LINE__);
            ...
            if(!empty($result)){
                ...
                $query1 = "SELECT `id`,  `thumb`,  `secret_id` FROM `cms_projects_photo` S WHERE S.`p_id`=:p_id ORDER BY `ordering`";
...
//            exit;
            ...
            if (!empty($data)){
                ...
                if($result === 1){
                    ...
                    $deleteResult = $this->db->delete('cms_projects_text',"`p_id` ={$id}");
    ...
    public function uploadPhoto(){
        try{
            if(!empty($_FILES['file']) && isset($_POST['sid']) && $_POST['sid']!='undefined'){
                ...
                $query = "SELECT `thumb`, `photo` FROM `cms_projects_photo` WHERE `p_id`= :p_id and date(`create_date`) < CURDATE()";
                $result = $this->db->select($query, $selectData);
                Func::dbError($result, __LINE__);
                ...
                if(!empty($result)){
                    ...
                    $resultDelete = $this->db->delete("`cms_projects_photo`", "`p_id`='0' and date(`create_date`) < CURDATE()");
                ...
                $query = "SELECT `ordering` FROM `cms_projects_photo` WHERE `secret_id`= :secret_id ORDER BY `ordering` desc LIMIT 1";
                                ...
                                if (in_array($file['type'], $allowedContentType)) {
                                    if (in_array($imageinfo['mime'], $allowedContentType)) {
                                        ...
                                        if ($handle->uploaded) {
                                            ...
                                            $handle->image_x            = projects_width;
                                            $handle->image_ratio_y      = projects_height;
                                            ...
                                            $handle->image_x            = projects_thumb_width;
                                            $handle->image_y            = projects_thumb_height;
                                            $handle->dir_chmod          = 0777;
                                            $handle->process(UPLOAD_DIR . 'Image/'.$this->_menu.'/');
                                            if ($handle->processed) {
                                                $handle->clean();
                                        ...
                                        $last_id = $this->db->insert('cms_projects_photo', $insertData);
                                        Func::dbError($last_id, __LINE__);
    ...
    public function photoDelete($id, $sec_id) {
        ...
        try{
            ...
            if(isset($result) && !empty($result)){
                $updateResult = $this->db->delete('`cms_projects_photo`', "`id` = '{$id}' and `secret_id`='{$sec_id}'");
                Func::dbError($updateResult, __LINE__);
        ...
        try{
            if(isset($_POST['action']) && $_POST['action'] = 'updateRecordsListings'){
                ...
                foreach ($_POST['recordsArray'] as $recordIDValue) {
                    ...
                    $result = $this->db->update('`cms_projects_photo`', $updateData, "`id`={$recordIDValue}");
                    Func::dbError($result, __LINE__);
                    $ordering++;
                }
            }
