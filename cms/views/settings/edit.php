                            ...
                            <div>
                                ...
                                <div class="tab-content">
                                    ...
                                    foreach (MFAdmin::$_langs as $key=>$value){
                                        ...
                                        $text = 'data_text_'.$key;
                                        $title = (isset($data->$title) && !empty($data->$title))?$data->$title:(isset($this->item['title'][$key])?$this->item['title'][$key]:'');
                                        $text = (isset($data->$text) && !empty($data->$text))?$data->$text:(isset($this->item['text'][$key])?$this->item['text'][$key]:'');
                                        ?>
                                        <div role="tabpanel" class="tab-pane<?=($x==1)?' active':''?>" id="tab_<?=$key?>">
                                            <div class="form-group">
                                                <label><?= Lang::get('{Açar söz və ya ifadə}') ?> *</label>
                                                <input class="form-control" name="data_title_<?=$key?>" value="<?=$title?>">
                                                <?=(isset($error))?Func::showError('data_title_'.$key, $error):''?>
                                            </div>
                                            <div class="form-group">
                                                <label><?= Lang::get('{Mətn}') ?> *</label>
                                                <textarea class="form-control" name="data_text_<?=$key?>"><?=$text?></textarea>
