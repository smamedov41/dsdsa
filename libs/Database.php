...
//            die('ERROR: '. $e->getMessage());
    ...
    public function select($sql, $array = array(), $fetchMode = PDO::FETCH_ASSOC) {
        try {
            ...
            if (!$sth->execute()) {
//                $error = $sth->errorInfo();
//                throw new Exception(json_encode(['errorMysql'=>$error[2]]));
            } else {
                return $sth->fetchAll($fetchMode);
            }
        } catch (Exception $e){
            return ['errorMysql'=>$e->getMessage()];
        }
    }
...
//                throw new Exception(json_encode(['errorMysql'=>$error[2]]));
    ...
    public function selectCount($sql, $array = array(), $fetchMode = PDO::FETCH_ASSOC) {
        try {
            ...
            if (!$sth->execute()) {
//                $error = $sth->errorInfo();
//                throw new Exception(json_encode(['errorMysql'=>'Count error']));
            } else {
...
//                throw new Exception(json_encode(['errorMysql'=>$error[2]]));
    ...
    public function update($table, $data, $where) {
        try {
            ...
            if (!$sth->execute()) {
//                $error = $sth->errorInfo();
//                throw new PDOException(json_encode(['errorMysql'=>$error[2]]));
            } else {
                return 1;
            }
